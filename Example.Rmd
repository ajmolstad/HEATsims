...
Now, we run the full version of HEAT using the function \texttt{penMLE\_CV} based on five-fold cross-validation. This will take a few minutem, so please exercise patience.  
```{r, cache=TRUE}
...
```
Now, let us extract the estimated $\beta_{\rm (AA)}$ and $\beta_{\rm (EA)}$. 
```{r, cache=TRUE}
...
sum((hat.beta.EA - beta.EA)^2)
```
One could also use error isolated to each population (AA or EA), though we do not recommend this. 


## Approximate version of HEAT
Now, we show how to implement the approximation version of HEAT. This will require using the \texttt{natural} package in R.
```{r, cache=TRUE}
library(natural)
