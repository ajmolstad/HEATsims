...
penMLE_path <- function(prot.AA, prot.EA, X.AA, X.EA, lambda, gamma, 
  ...
  if(is.null(prot.AA.test)){
    for(kk in 1:length(gamma)){
      ...
      rhoE[kk] <- fit.temp$rhoE
      
      rhoA.temp <- fit.temp$rhoA
  ...
  if(!is.null(prot.AA.test)){
    ...
    for(kk in 1:length(gamma)){
        ...
        phiA.temp <- fit.temp$phiA
        phiE.temp <- fit.temp$phiE
        err.AA[kk] <- sum((prot.AA.test - mean(prot.AA) - X.AA.test.stand%*%phiA[,kk]/rhoA[kk])^2)
    ...
    list("phiA" = phiA, "phiE" = phiE, "rhoA" = rhoA, "rhoE"= rhoE)
  }
...
penMLE_CV <- function(prot.AA, prot.EA, X.AA, X.EA, delta = 0.1, nlambda = 10, 
  ...
  w <- w/max(w)
  rhoA <- 1/sd(prot.AA.stand)
  rhoE <- 1/sd(prot.EA.stand)
  ...
  if(!is.null(nfolds)){
    ...
    for(kk in 1:nfolds){
      for(ll in 1:nlambda){
        cv.fit <- penMLE_path(prot.AA[-which(fold.id.AA == kk)], 
                                      prot.EA[-which(fold.id.EA == kk)], 
                                      X.AA[-which(fold.id.AA == kk),], 
                                      X.EA[-which(fold.id.EA == kk),], 
                                      lambda[ll], gamma.mat[which(t0[ll,]!=Inf),ll], 
                                      prot.AA[which(fold.id.AA == kk)], 
                                      prot.EA[which(fold.id.EA == kk)], 
                                      X.AA[which(fold.id.AA == kk),], 
    ...
    full.fit <- list()
    for(ll in 1:nlambda){
      full.fit[[ll]] <- penMLE_path(prot.AA, 
                                      prot.EA, 
                                      X.AA, 
                                      X.EA, 
                                      lambda[ll], 
                                      gamma.mat[which(t0[ll,]!=Inf),ll], w=w)
    }
    return(list("errs.AA" = errs.AA, "errs.EA" = errs.EA, "L.errs.AA" = L.errs.AA, "L.errs.EA" = L.errs.EA, 
      "full.fit" = full.fit, 
    ...
    return(list("full.fit" = full.fit, "lambda" = lambda, "gamma.mat" = gamma.mat))
  }
}
